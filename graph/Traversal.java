...
public abstract class Traversal {
    ...
    protected Traversal(Graph G, Queue<Integer> fringe) {
        _G = G;
        _fringe = fringe;
        _mark = new boolean[G.maxVertex() + 1];
        _postMark = new boolean[G.maxVertex() + 1];
    }
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            int v = _fringe.poll();
            if (marked(v)) {
                if (shouldPostVisit(v) && !_postMark[v]) {
                    postVisit(v);
                    _postMark[v] = true;
                }
            } else {
    ...
    /**
     * Array storing if it has been visited.
     */
    private boolean[] _mark;

    /**
     * Array storing if it has been visited(for postOrder Traversal).
     */
    private boolean[] _postMark;
