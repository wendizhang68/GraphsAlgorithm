...
class Maker {
    ...
    void readFileAges(String fileInfoName) {
        ...
        try {
            ...
            error("error: Could not find fileInfo: %s", fileInfoName);
    ...
    void build(String target) {
        Rule targetRule = addRule(target, Collections.<String>emptyList(),
                Collections.<String>emptyList());
        if (targetRule == null) {
            error("error: no target to build");
        }
    ...
    class MakeTraversal extends DepthFirstTraversal {
        ...
        protected boolean postVisit(int v0) {
            for (int u : _depends.successors(v0)) {
                Rule n = _depends.getLabel(u);
                if (n != null && n.isUnfinished()) {
                    error("error: There is circular dependency.");
                }
            }
